...
def main(args):
    ...
    val_loader = DataLoader(
        valid_dataset, batch_size=batch_size, num_workers=workers, shuffle=True
    )
    ...
    model = get_model(
        args.model, inchannels=inchannels, pretrained=not args.no_pretrained
    ).to(device)
    ...
    for epoch in range(start_epoch, n_epochs + 1):
        ...
        if valloss <= best_loss or kappa >= best_kappa:
            save_msg = (
                f"saving model to {snapshot_path}"  # add this message if model saved
            )
