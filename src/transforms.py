...
import torch
import random
from src.data import l2abands as bands
    ...
    def data_augmentation(image, mask):
        ...
        if intensity >= 1:
            ...
            if random.random() < 0.5:
                ...
                pixel_noise = torch.repeat_interleave(
                    pixel_noise.unsqueeze(0), image.size(0), dim=0
                )
            ...
            if random.random() < 0.5:
                # add noise
                noise = (
                    torch.rand(image.shape[0], image.shape[1], image.shape[2])
                    * std_noise
                )
                image = image + noise
