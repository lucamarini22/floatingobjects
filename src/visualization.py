...
def plot_batch(images, masks, y_preds):
    ...
    height = 3
    width = 3
    fig, axs = plt.subplots(N, 5, figsize=(5 * width, N * height), squeeze=False)
...
def plot_curves(nets, fpr, tpr, roc_auc, recall, prec):
    ...
    colors = (
        cycle(["aqua", "red"])
        if len(nets) == 2
        else cycle(["aqua", "red", "cornflowerblue", "moccasin", "mediumpurple"])
    )
    ...
    if "svm" in fpr:
        ax1.plot(
            fpr["svm"],
            tpr["svm"],
            label="Support-Vector Machine (AUC = {0:0.2f})".format(roc_auc["svm"]),
            color="deeppink",
            linewidth=lw,
        )
        ax1.plot(
            fpr["rf"],
            tpr["rf"],
            label="Random Forest (AUC = {0:0.2f})".format(roc_auc["rf"]),
            color="navy",
            linewidth=lw,
        )
        ax1.plot(
            fpr["nb"],
            tpr["nb"],
            label="Naïve Bayes (AUC = {0:0.2f})".format(roc_auc["nb"]),
            color="forestgreen",
            linewidth=lw,
        )
        ax1.plot(
            fpr["hgb"],
            tpr["hgb"],
            label="Hist. Gradient Boosting (AUC = {0:0.2f})".format(roc_auc["hgb"]),
            color="darkorange",
            linewidth=lw,
        )
    ...
    for i, color in zip(nets, colors):
        ...
        ax1.plot(
            ...
            color=color,
            lw=lw,
            label="{0} (AUC = {1:0.2f})".format(j, roc_auc[j]),
        )
    ...
    if "svm" in fpr:
        ...
        ax2.plot(
            recall["rf"], prec["rf"], label="Random Forest", color="navy", linewidth=lw
        )
        ax2.plot(
            recall["nb"],
            prec["nb"],
